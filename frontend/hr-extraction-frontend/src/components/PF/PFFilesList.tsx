...
//       const numericInput = removeCommas(input);
//       if (numericInput === "" || /^\d*\.?\d*$/.test(numericInput)) {
//         const formatted = formatNumberWithCommas(numericInput);
//         setRemittanceAmount(formatted);
//       }
//     };

//     const handleSubmit = (e: React.FormEvent) => {
//       e.preventDefault();
//       const cleanAmount = removeCommas(remittanceAmount);
//       console.log("Subitted Amount:", cleanAmount);
//     };
//   };
...
const PFFilesList: React.FC = () => {
  ...
  const handleRemittanceMonthChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const inputValue = e.target.value;
    setRemittanceMonthInput(inputValue);
    setRemittanceMonth(formatDateForBackend(inputValue));
  };
  ...
  const RemittanceUploadForm = React.useCallback(({ fileId }: { fileId: number }) => {
          ...
          <table className="files-table">
            <thead>
              <tr>
                ...
                <th>Remittance</th>
                {user?.role === "admin" && !selectedUserId && <th>Submitted BY</th>}
                <th>Actions</th>
              </tr>
            </thead>
            <tbody>
              {filteredAndSortedFiles.map((file) => (
                <tr key={`pf-${file.id}`} className={file.status}>
                  ...
                  {user?.role === "admin" && !selectedUserId && (
                    <td>{users.find((u) => u.id === file.user_id)?.name || "Unknown User"}</td>
                  )}
                  <td className="actions">
                    {file.status === "success" ? (
                      <div className="download-buttons">
